...
def run_langgraph_planning(iterations: int = BENCHMARK_ITERATIONS) -> dict:
...
def run_apxm_planning(iterations: int = BENCHMARK_ITERATIONS) -> dict:
...
def main():
    ...
    try:
        results["results"]["langgraph"] = run_langgraph_planning(args.iterations)
    except ImportError as e:
        results["results"]["langgraph"] = {"error": str(e)}

    # Run A-PXM benchmark
    results["results"]["apxm"] = run_apxm_planning(args.iterations)
