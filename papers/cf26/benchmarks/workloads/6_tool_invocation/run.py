#!/usr/bin/env python3
"""
Tool Invocation Benchmark Runner

Compares A-PXM's native capability system vs LangGraph's runtime tool binding.
...
def run_langgraph_tools(iterations: int = BENCHMARK_ITERATIONS, warmup: int = WARMUP_ITERATIONS) -> dict:
    """Run LangGraph tool workflow and measure timing."""
...
def run_apxm_tools(iterations: int = BENCHMARK_ITERATIONS, warmup: int = WARMUP_ITERATIONS) -> dict:
    """Run A-PXM tool workflow through the REAL pipeline."""
...
def run_langgraph(iterations: int = BENCHMARK_ITERATIONS, warmup: int = WARMUP_ITERATIONS) -> dict:
    ...
    return run_langgraph_tools(iterations, warmup=warmup)


def run_apxm(iterations: int = BENCHMARK_ITERATIONS, warmup: int = WARMUP_ITERATIONS) -> dict:
    """Entry point for the suite runner (`runner.py`)."""
    return run_apxm_tools(iterations, warmup=warmup)


def main():
    parser = argparse.ArgumentParser(description="Tool Invocation Benchmark")
    ...
    try:
        results["results"]["langgraph"] = run_langgraph_tools(args.iterations, warmup=args.warmup)
    except ImportError as e:
        results["results"]["langgraph"] = {"error": str(e)}

    # Run A-PXM benchmark
    results["results"]["apxm"] = run_apxm_tools(args.iterations, warmup=args.warmup)

    if args.json:
        print(json.dumps(results, indent=2))
    else:
        print(f"\nTool Invocation Benchmark Results")
        print(f"{'=' * 50}")
        print(f"Query: {QUERY}")
        ...
        print(f"A-PXM (with capability system):")
        if apxm.get("success"):
