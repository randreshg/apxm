...
def run_langgraph_multiagent(iterations: int = BENCHMARK_ITERATIONS, warmup: int = WARMUP_ITERATIONS) -> dict:
...
def run_apxm_multiagent(iterations: int = BENCHMARK_ITERATIONS, warmup: int = WARMUP_ITERATIONS) -> dict:
...
def run_langgraph(iterations: int = BENCHMARK_ITERATIONS, warmup: int = WARMUP_ITERATIONS) -> dict:
    ...
    return run_langgraph_multiagent(iterations, warmup=warmup)


def run_apxm(iterations: int = BENCHMARK_ITERATIONS, warmup: int = WARMUP_ITERATIONS) -> dict:
    """Entry point for the suite runner (`runner.py`)."""
    return run_apxm_multiagent(iterations, warmup=warmup)
...
def main():
    ...
    try:
        results["results"]["langgraph"] = run_langgraph_multiagent(args.iterations, warmup=args.warmup)
    except ImportError as e:
        results["results"]["langgraph"] = {"error": str(e)}

    # Run A-PXM benchmark
    results["results"]["apxm"] = run_apxm_multiagent(args.iterations, warmup=args.warmup)
