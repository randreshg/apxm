...
Agents are represented structurally at runtime via `Agent` and `AgentFlow`
types. Artifact loading reconstructs these runtime agents and registers them
through `FlowRegistry::register_agent()`, which stores:
...
The callee flow DAG is scheduled alongside the caller's, enabling the scheduler to
...
1. Each agent's flow DAGs are merged into a **unified scheduler graph**.
...
flowchart LR
    subgraph Agent_A ["Agent A"]
        subgraph Flow_A ["Flow: main"]
            A1["Codelet: Fetch data"] --> A2["Codelet: Summarize"]
        end
    end

    subgraph Agent_B ["Agent B"]
        subgraph Flow_B ["Flow: main"]
            B1["Codelet: Fetch data"] --> B2["Codelet: Analyze"]
        end
    end

    subgraph Agent_C ["Agent C"]
        subgraph Flow_C ["Flow: main"]
            C1["Codelet: Merge results"] --> C2["Codelet: Report"]
        end
    end
...
- **FLOW as inlined flow DAG** means cross-agent calls have the same overhead as
