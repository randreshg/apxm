...
/// Inner plan DSL code from LLM
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct InnerPlanDsl {
    /// APxM DSL code to be compiled into an ExecutionDAG
    pub dsl: String,
}
...
pub async fn execute(ctx: &ExecutionContext, node: &Node, _inputs: Vec<Value>) -> Result<Value> {
    ...
    for attempt in 0..=3 {
        ...
        match execute_plan_once(ctx, node, &request, enable_inner_plan, &goal).await {
...
async fn execute_plan_once(
    ...
    if let Ok(plan_output) = parse_plan_output(&content) {
        ...
        if enable_inner_plan && plan_output.inner_plan.is_some() {
            tracing::info!(
                execution_id = %ctx.execution_id,
                "Executing inner plan"
            );

            // TODO: Execute inner DAG
            // This would require access to the Runtime/Executor
            // For now, we just note that an inner plan exists
            ctx.memory
                .write(
                    crate::memory::MemorySpace::Episodic,
                    format!("inner_plan_requested:{}", ctx.execution_id),
                    Value::String(
                        "Inner plan execution requested but not yet implemented".to_string(),
                    ),
                )
                .await
                .ok();
