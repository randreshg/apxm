...
pub async fn execute(ctx: &ExecutionContext, node: &Node, inputs: Vec<Value>) -> Result<Value> {
    ...
    let sub_dag_value = if let Some(idx) = matched_index {
        ...
        node.attributes
            .get(graph_attrs::CASE_REGIONS)
            .and_then(|v| v.as_array())
            .and_then(|arr| arr.get(idx))
            .cloned()
    } else {
        // Use default region
        node.attributes.get(graph_attrs::DEFAULT_REGION).cloned()
    };
...
fn parse_node(value: &Value) -> Result<Option<apxm_core::types::execution::Node>> {
    ...
    let op_type = AISOperationType::from_wire_index(op_type_num as u32).ok_or_else(|| {
