...
std::optional<StringRef> mapAisOpToRustType(Operation *op) {
  return TypeSwitch<Operation *, std::optional<StringRef>>(op)
      ...
      .Case<func::ReturnOp>([](auto) { return "AISOperationType::Return"; })
...
LogicalResult emitModule(ModuleOp module, llvm::raw_ostream &os,
  ...
  os << "use std::path::Path;\n";
  os << "use apxm_core::types::{AISOperationType, DependencyType, Edge, ExecutionDag, Node, NodeMetadata, Number, Value};\n";
  os << "use apxm_artifact::{Artifact, ArtifactMetadata};\n";
  os << "use apxm_runtime::{Runtime, RuntimeConfig, RuntimeResult};\n\n";
  ...
  if (options.emitMainFunction) {
    ...
    os.indent(options.indentSize)
        << "if let Ok(path) = std::env::var(\"APXM_ARTIFACT_OUT\") {\n";
    os.indent(options.indentSize + options.indentSize)
        << "let metadata = ArtifactMetadata::new(dag.metadata.name.clone(), &dag);\n";
    os.indent(options.indentSize + options.indentSize)
        << "let artifact = Artifact::new(metadata, dag.clone());\n";
    os.indent(options.indentSize + options.indentSize)
        << "artifact\n";
    os.indent(options.indentSize + options.indentSize)
        << "    .write_to_path(Path::new(&path))\n";
    os.indent(options.indentSize + options.indentSize)
        << "    .map_err(|e| apxm_runtime::RuntimeError::State(e.to_string()))?;\n";
    os.indent(options.indentSize + options.indentSize)
        << "return Ok(());\n";
    os.indent(options.indentSize) << "}\n";
    os.indent(options.indentSize)
    ...
    os.indent(options.indentSize)
        << "let _report = runtime.execute(dag).await?;\n";
